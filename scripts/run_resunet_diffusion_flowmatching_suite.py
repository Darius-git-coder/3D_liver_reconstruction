...
def main() -> None:
    ...
    manifest: Dict[str, Any] = {"config": args.config, "commands": []}
    for name, method_values in iter_methods(config):
        values = merged(common, method_values)
        train_out = os.path.join(root_out, name, "train")
        eval_out = os.path.join(root_out, name, "eval")

        if name == "resunet":
            train_cmd = build_resunet_train(python_exe, data, train_out, values)
            eval_cmd = build_resunet_eval(python_exe, data, train_out, eval_out, values)
        else:
            train_cmd = build_generative_train(python_exe, name, data, train_out, values)
            eval_cmd = build_generative_eval(python_exe, name, data, train_out, eval_out, values)

        if not args.skip_train:
            manifest["commands"].append({"method": name, "stage": "train", "command": train_cmd})
            run_command(train_cmd, dry_run=args.dry_run)
        if not args.skip_eval:
            manifest["commands"].append({"method": name, "stage": "eval", "command": eval_cmd})
